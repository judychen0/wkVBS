...
void xSkim_mc(char* pathes, char* PUpathes, char* IDpathes, char* PSVpathes, Int_t dcount, Int_t year, Int_t sig, Int_t ULsamples, Int_t isSM){
  ...
  // MET comparison of pf & puppi
  TH1F *h_pfMET_all;
  TH1F *h_pfMET_HLTpass;
  TH1F *h_puppiMET_all;
  TH1F *h_puppiMET_HLTpass;

  h_pfMET_all = new TH1F("h_pfMET_all", "", 10, METbin);
  h_pfMET_HLTpass = new TH1F("h_pfMET_HLTpass", "", 10, METbin);
  h_puppiMET_all = new TH1F("h_puppiMET_all", "", 10, METbin);
  h_puppiMET_HLTpass = new TH1F("h_puppiMET_HLTpass", "", 10, METbin);  
  h_pfMET_all->Sumw2();
  h_pfMET_HLTpass->Sumw2();
  h_puppiMET_all->Sumw2();
  h_puppiMET_HLTpass->Sumw2();
  ...
  for (Long64_t ev = 0; ev < data.GetEntriesFast(); ev++){
    ...
    // PF & PUPPI MET comparison
    h_pfMET_all->Fill(corrMET);
    h_puppiMET_all->Fill(puppiMET);
    ...
    for(Int_t ipho=0; ipho<nPho; ipho++){
      ...
      if(dcount == 0 && sig == 0 && phoIsMatch[ipho] == 0) bit = SetBit(0, bit);
      if(fabs(phoSCEta[ipho]) < 1.4442 && phohasPixelSeed[ipho] == 0)							bit = SetBit(1, bit);
      if(isSM == 0){
	if(phoEt[ipho]>75) bit = SetBit(2, bit);
      }
      else{
	if(phoEt[ipho]>210) bit = SetBit(2, bit);
      }
      if(MphoID[ipho] == 1)								bit = SetBit(3, bit);
      if(fabs(phoSeedTime[ipho])<3 && phoSigmaIEtaIEtaFull5x5[ipho] > 0.001 && phoMIPTotEnergy[ipho] < 4.9) bit = SetBit(4, bit);
	...
	if(corrMET>100) bit = SetBit(6, bit);
      }
      if(fabs(phoMETdPhi[ipho]) > 1.2)							bit = SetBit(7, bit);
      
      if(npfjet <= 1 ){
	bit = SetBit(8, bit); nSMjet = npfjet;
      }
      ...
      for(Int_t i=0; i<9; i++){								bit = ResetBit(i+7, bit); }
      bit = SetBit(7, bit);
      bit = SetBit(8, bit); //njet cut
      if(npfjet >=2){
	nVBSjet = npfjet;
	Int_t ilead = nonPUjetid[0];
	Int_t isub = nonPUjetid[1];
