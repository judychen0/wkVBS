...
void xSkim_mc(char* pathes, char* PUpathes, char* IDpathes, char* PSVpathes, Int_t dcount, Int_t year, Int_t sig, Int_t ULsamples, Int_t isSM){
  ...
  for (Long64_t ev = 0; ev < data.GetEntriesFast(); ev++){
    ...
    TVector2 METv2;
    if(ULsamples == 1) METv2.SetMagPhi(puppiMET, puppiMETPhi);
    //if(ULsamples == 1 && sig == 0) METv2.SetMagPhi(puppiMET, puppiMETPhi);
    else METv2.SetMagPhi(pfMET, pfMETPhi);
    TVector2 corrMETv2;
    ...
    // PF & PUPPI MET comparison
    if((HLTJet_>>27&1) == 1){
      h_pfMET_HLTpass->Fill(corrMET);
      h_puppiMET_HLTpass->Fill(puppiMET);
    }
	...
	if(ULsamples == 0)PSVSF = h2_PSVSF->GetBinContent(1, 1);// Legacy MC
      ...
      Double_t tot_wei = 1;
      tot_wei = phoSF*puwei_*genWeightSign*LumiWei;
      EventWeight = tot_wei;
    }
    ...
    for(Int_t ipho=0; ipho<nPho; ipho++){
      ...
      //else if(dcount == 0 && sig == 0)							bit = SetBit(0, bit);
