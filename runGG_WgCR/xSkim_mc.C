...
void xSkim_mc(char* pathes, char* PUpathes, char* IDpathes, char* PSVpathes, Int_t dcount, Int_t year, Int_t sig, Int_t ULsamples){
  ...
  vector<Int_t> MCTruePhoID;
  vector<Int_t> MCFakePhoID;
  ...
  TH1F *h_lepMET_mT = new TH1F("h_lepMET_mT", "", 50, 0, 1000);
  h_lepMET_mT->Sumw2();
  TH1F *h_lepMET_RT = new TH1F("h_lepMET_RT", "", 50, 0, 1000);
  h_lepMET_RT->Sumw2();
  ...
  for (Long64_t ev = 0; ev < data.GetEntriesFast(); ev++){
    ...
    for(Int_t ijet=0; ijet<nJet_; ijet++){
      //if(year == 2017 && UL dijesamples == 0 && jetPt_[ijet]<50 && fabs(jetEta_[ijet])>2.56 && fabs(jetEta_[ijet])<3.139) jetPt_[ijet] = 0;//remove 2017 ecal noise
    ...
    TVector2 METv2;
    //if(ULsamples == 1) METv2.SetMagPhi(puppiMET_, puppiMETPhi_);
	...
	lepMET_RTv2 = lepv2 + corrMETv2;
	lepMET_RT = lepMET_RTv2.Mod();

	lepMET_mT = TMath::Sqrt(pow(elePt_[index_ele],2) + pow(corrMET,2) + pow(Wmass,2));
      }
      else if(nLmu == 1){
	...
	//lepMET_mT = muPt_[index_mu] + corrMET;
	lepMET_mT = TMath::Sqrt(pow(muPt_[index_mu],2) + pow(corrMET,2) + pow(Wmass,2));
      }

      // h_lepMET_mT->Fill(lepMET_mT);
      // h_lepMET_RT->Fill(lepMET_RT);
      // cout << "lepMET_RT : " << lepMET_RT << " lepMET_mT : " << lepMET_mT << endl;
    }
    ...
    for(Int_t ipho=0; ipho<nPho; ipho++){
      ...
      // 	// if(isEle==1 && nLele==1){
      // 	//   bit = SetBit(5, bit);
      // 	//   //if((corrMET+elePt_[index_ele]) > 120) bit = SetBit(6,bit);
      // 	// }
      // 	// else if(isEle==0 && nLmu==1){
      // 	//   bit = SetBit(5, bit);
      // 	//   //if((corrMET+muPt_[index_mu]) > 120) bit = SetBit(6,bit);
      // 	// }
      // }
      // if(corrMET > 150) bit = SetBit(6, bit);
      //bit = SetBit(6, bit);
