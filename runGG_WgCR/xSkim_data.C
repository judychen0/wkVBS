...
void xSkim_data(char* pathes, Int_t year, Int_t isMET){
  ...
  sprintf(foutname, "WgCR_%s", treename.c_str());
...
// photon
  ...
  TH1F *hbin_WgCR_dijetMass[Nbin];
  TH1F *hbin_WgCR_dijetMass_cutsieie[Nbin];
  ...
  for(Int_t jj=0; jj<Nbin; jj++){
    hbin_WgCR_dijetMass[jj] = new TH1F(Form("hbin_WgCR_dijetMass_%i", jj+1), "", 4, massbin);
    hbin_WgCR_dijetMass_cutsieie[jj] = new TH1F(Form("hbin_WgCR_dijetMass_cutsieie_%i", jj+1), "", 4, massbin);
    hbin_WgCR_dijetMass[jj]->Sumw2();
    hbin_WgCR_dijetMass_cutsieie[jj]->Sumw2();
  }
  ...
  for (Long64_t ev = 0; ev < data.GetEntriesFast(); ev++){
    ...
    //   Int_t boo = 0;
    //   if(elePt_[iele]>30 && isEle == 1){
    // 	if(year==2016 && (eleFiredSingleTrgs_[iele]>>12&1) == 0) boo = 1;
    // 	else if(year==2017 && (eleFiredSingleTrgs_[iele]>>13&1) == 0) boo = 1;
    // 	else if(year==2018 && (eleFiredSingleTrgs_[iele]>>13&1) == 0) boo = 1;
    //   }
    //   else if(elePt_[iele]<30 && isEle == 1){
    // 	boo = 1;
    //   }
    //   if(boo == 1) continue;
      
    //   /// tighter ele selection ///
    //   if((eleIDbit_[iele]>>1&1) == 1 && elePt_[iele] > 30 && fabs(eleEta_[iele]) < 2.5) {
    //   	if(fabs(eleEta_[iele]) <1.4442 && eleD0_[iele] < 0.5 && eleDz_[iele] < 1.0){
    // 	  nLele++;
    // 	  index_ele = iele;
    // 	  if(nLele>0) break;
    // 	}
    //   	else if(fabs(eleEta_[iele]) > 1.5666 && eleD0_[iele] < 1.0 && eleDz_[iele] < 2.0){
    // 	  nLele++;
    // 	  index_ele = iele;
    // 	  if(nLele>0) break;
    // 	}
    ...
    //   if((muIDbit_[imu]>>0&1) == 1 && muPt_[imu] > 20 && fabs(muEta_[imu]) < 2.4){
    //   	if(muD0_[imu] < 2 && muDz_[imu] < 5){
    // 	  nLmu++;
    // 	  index_mu = imu;
    // 	  if(nLmu>0) break;
    // 	}
    ...
    for(Int_t iele=0; iele<nEle_; iele++){
      ...
      /// tighter ele selection ///
      if((eleIDbit_[iele]>>1&1) == 1 && elePt_[iele] > 30 && fabs(eleEta_[iele]) < 2.5) {
      	if(fabs(eleEta_[iele]) <1.4442 && fabs(eleD0_[iele]) < 0.05 && fabs(eleDz_[iele]) < 0.1){
	  nLele++;
	  index_ele = iele;
	  if(nLele>0) break;
	}
      	else if(fabs(eleEta_[iele]) > 1.5666 && fabs(eleD0_[iele]) < 0.1 && fabs(eleDz_[iele]) < 0.2){
	  nLele++;
	  index_ele = iele;
	  if(nLele>0) break;
	}
    ...
    for(Int_t imu=0; imu<nMu_; imu++){
      ...
      /// tighter mu selection///
      if((muIDbit_[imu]>>0&1) == 1 && muPt_[imu] > 30 && fabs(muEta_[imu]) < 2.4){
      	if(fabs(muD0_[imu]) < 0.2 && fabs(muDz_[imu]) < 0.5){
	  nLmu++;
	  index_mu = imu;
	  if(nLmu>0) break;
	}
    ...
    // CR variables
    TVector2 lepMET_RTv2;
    TVector2 puppiMETv2;
    puppiMETv2.SetMagPhi(puppiMET, puppiMETPhi);
    Float_t lepMET_RT = 0;
    Float_t lepMET_mT = 0;
    if(nLep == 1){
      TVector2 lepv2;
      if(nLele == 1){
	lepv2.SetMagPhi(elePt_[index_ele], elePhi_[index_ele]);
	lepMET_RTv2 = lepv2 + puppiMETv2;
	lepMET_RT = lepMET_RTv2.Mod();

	lepMET_mT = elePt_[index_ele] + puppiMET;
      }
      else if(nLmu == 1){
	lepv2.SetMagPhi(muPt_[index_mu], muPhi_[index_mu]);
	lepMET_RTv2 = lepv2 + puppiMETv2;
	lepMET_RT = lepMET_RTv2.Mod();

	lepMET_mT = muPt_[index_mu] + puppiMET;
      }
    }
    ...
    for(Int_t ipho=0; ipho<nPho; ipho++){
      ...
      // 	// if(isEle==1 && nLele==1){
      // 	//   bit = SetBit(5, bit);
      // 	//   //if((puppiMET+elePt_[index_ele]) >120) bit = SetBit(6,bit);
      // 	// }
      // 	// else if(isEle==0 && nLmu==1){
      // 	//   bit = SetBit(5, bit);
      // 	//   //if((puppiMET+muPt_[index_mu]) >120) bit = SetBit(6,bit);
      // 	// }
      // 	bit = SetBit(5,bit);
      // }
      // if(puppiMET > 150) bit = SetBit(6, bit);
      //bit = SetBit(6, bit);
      if(nLep == 1 && lepMET_mT <200) bit = SetBit(5,bit);
      if(lepMET_RT >180) bit = SetBit(6,bit);
	...
	if((phoEt[ipho]/lepMET_RT) < 2.4 ) bit = SetBit(13, bit);
	if(fabs(minJMETdPhi)   > 0.5)							bit = SetBit(14, bit);
	if(dijetMass>250)								bit = SetBit(15, bit);
      }

      // fill dijetMass, sieie loosed
      if(pho_sel(bit,15) == 1){
	for(Int_t ibin=0; ibin<Nbin; ibin++){
	  if(phoEt[ipho] >= ptbin[ibin] && phoEt[ipho] < ptbin[ibin+1] && ipho == 0){
	    // if(isEle==1) hbin_Wenug_dijetMass[ibin]->Fill(dijetMass, EventWeight);
	    // else hbin_Wmunug_dijetMass[ibin]->Fill(dijetMass, EventWeight);
	    hbin_WgCR_dijetMass[ibin]->Fill(dijetMass, EventWeight);
	  }
	}
      }
	...
	for(Int_t ibin=0; ibin<Nbin; ibin++){
	  if(phoEt[ipho] >= ptbin[ibin] && phoEt[ipho] < ptbin[ibin+1] && ipho == 0){
	    // if(isEle==1) hbin_Wenug_dijetMass_cutsieie[ibin]->Fill(dijetMass, EventWeight);
	    // else hbin_Wmunug_dijetMass_cutsieie[ibin]->Fill(dijetMass, EventWeight);
	    hbin_WgCR_dijetMass_cutsieie[ibin]->Fill(dijetMass, EventWeight);
	  }
	}
      }
  ...
  // fout_->mkdir("Wenug_bin_template");
  // fout_->cd("Wenug_bin_template");
  // for(Int_t jj=0; jj<Nbin; jj++){
  //   hbin_Wenug_dijetMass[jj]->Write();
  //   hbin_Wenug_dijetMass_cutsieie[jj]->Write();
  // }

  // fout_->mkdir("Wmunug_bin_template");
  // fout_->cd("Wmunug_bin_template");
  // for(Int_t jj=0; jj<Nbin; jj++){
  //   hbin_Wmunug_dijetMass[jj]->Write();
  //   hbin_Wmunug_dijetMass_cutsieie[jj]->Write();
  // }

  for(Int_t jj=0; jj<Nbin; jj++){
    hbin_WgCR_dijetMass[jj]->Write();
    hbin_WgCR_dijetMass_cutsieie[jj]->Write();
  }
