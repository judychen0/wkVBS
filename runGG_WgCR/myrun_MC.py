...
#isEle = raw_input("ElectronCR or MuonCR? (Ele==1; Mu==0): ")
...
#if int(isEle)==1: foldername = "WenugCR_MC"
#else: foldername = "WmunugCR_MC"
foldername = "WgCR_MC"
...
with open(remotepath, "r") as filepath:
    ...
    for line in path:
        ...
        script += "#!/bin/bash \n"
        script += "\n"
        script += "workdir=/home/jou/wkVBS/work/runGG_WgCR \n"
        script += "homedir=/home/jou/wkVBS/runGG_WgCR \n"
        script += "mkdir /data1/GMET/"+str(foldername)+"/"+str(yeardir)+"/"+str(dirname)+" \n"
        script += "returndir=/data1/GMET/"+str(foldername)+"/"+str(yeardir)+"/"+str(dirname)+" \n"
        ...
        script += """/usr/bin/time parallel --progress --jobs 40 'root -L -l -b -q "xSkim_mc.C+(\\\"{1}\\\", \\\"{2}\\\", \\\"{3}\\\", \\\"{4}\\\", {5}, {6}, {7}, {8})"' :::: path.txt ::: $PUpath ::: $IDSFpath ::: $PSVpath ::: $isdcount ::: $year ::: $isSig :::+ $UL  \n"""
        script += "echo Processed all $count ggtrees!!! \n"
        script += "for i in {0..9}; do hadd output_$i.root WgCR_ggtree_*$i.root; done \n"
        script += "rm -r *ggtree*  \n"
