...
void xZgdata1(char* pathes){
  ...
  for(Int_t j=0; j<2; j++){
    ...
    h_phoEB_pt_ptoverMET[j] = new TH1F(Form("h_phoEB_pt_ptoverMET_%i", j), "pho Pt over MET cut", 16, ptbin);
    ...
    h_MET_ptoverMET[j] = new TH1F(Form("h_MET_ptoverMET_%i", j), "pho Pt over MET cut", 10, METbin);
    ...
    h_MET_130[j]->Sumw2();
    h_MET_M[j]->Sumw2();
    h_MET_leptonveto[j]->Sumw2();
    h_MET_MET[j]->Sumw2();
    h_MET_dphoMETPhi[j]->Sumw2();
    h_MET_jetveto[j]->Sumw2();
    h_MET_jetpt[j]->Sumw2();
    h_MET_jetjetdEta[j]->Sumw2();
    h_MET_jetjetdPhi[j]->Sumw2();
    h_MET_phojetdR[j]->Sumw2();
    h_MET_ptoverMET[j]->Sumw2();
    h_MET_mindJMETPhi[j]->Sumw2();
    h_MET_dijetMass[j]->Sumw2();
  ...
  h_phoEB_ptcut_minJMETdPhi_SB = new TH1F("h_phoEB_ptcut_minJmetdPhi_SB", "pho pT in minJMETdPhi SB region", 16, ptbin);
  ...
  h_phoEB_ptcut_minJMETdPhi_SR = new TH1F("h_phoEB_ptcut_minJmetdPhi_SR", "pho pT in minJMETdPhi SR region", 16, ptbin);
  ...
  for (Long64_t ev = 0; ev < data.GetEntriesFast(); ev++){
	...
	if(pho_sel(cutflow[icat],3) == 1) h_phoEB_pt_130[icat]->Fill(phoEt[ipho], WEIGHT);
	...
	if(pho_sel(cutflow[icat],3) == 1) h_MET_130[icat]->Fill(puppiMET, WEIGHT);
	if(pho_sel(cutflow[icat],4) == 1) h_MET_M[icat]->Fill(puppiMET, WEIGHT);
	if(pho_sel(cutflow[icat],6) == 1) h_MET_leptonveto[icat]->Fill(puppiMET, WEIGHT);
	if(pho_sel(cutflow[icat],7) == 1) h_MET_MET[icat]->Fill(puppiMET, WEIGHT);
	if(pho_sel(cutflow[icat],8) == 1) h_MET_dphoMETPhi[icat]->Fill(puppiMET, WEIGHT);
	if(pho_sel(cutflow[icat],9) == 1) h_MET_jetveto[icat]->Fill(puppiMET, WEIGHT);
	if(pho_sel(cutflow[icat],10) == 1) h_MET_jetpt[icat]->Fill(puppiMET, WEIGHT);
	if(pho_sel(cutflow[icat],11) == 1) h_MET_jetjetdEta[icat]->Fill(puppiMET, WEIGHT);
	if(pho_sel(cutflow[icat],12) == 1) h_MET_jetjetdPhi[icat]->Fill(puppiMET, WEIGHT);
	if(pho_sel(cutflow[icat],13) == 1) h_MET_phojetdR[icat]->Fill(puppiMET, WEIGHT);
	if(pho_sel(cutflow[icat],14) == 1) h_MET_ptoverMET[icat]->Fill(puppiMET, WEIGHT);
	if(pho_sel(cutflow[icat],15) == 1) h_MET_mindJMETPhi[icat]->Fill(puppiMET, WEIGHT);
	if(pho_sel(cutflow[icat],16) == 1) h_MET_dijetMass[icat]->Fill(puppiMET, WEIGHT);
	...
	if(pho_sel(cutflow[icat],16) == 1) {
	  ...
	  if(icat == 1){
	    ...
	    h_ptoverMET_cut->Fill(phoEt[ipho]/puppiMET, WEIGHT);
	...
	//VBS Nm1 plots
	  ...
	  if(Nm1_sel(cutflow[icat],14) == 1) h_ptoverMET_Nm1->Fill(phoEt[ipho]/puppiMET, WEIGHT);
	  ...
	  if(Nm1_sel(cutflow[icat],14) == 1){
	    if(h_ptoverMET_cut->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) >= 12 && h_ptoverMET_cut->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) <= 20){//ptoverMET CR
	      ptoverMET_CR_count++;
	    ...
	    else if(h_ptoverMET_cut->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) >= 1 && h_ptoverMET_cut->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) <= 11){//ptoverMET SR
	    ...
	    for(Int_t jj=0; jj<npfjet; jj++){
	      Int_t ijet = nonPUjetid[jj];
	      if(jj>1) continue;
	      if(h_ptoverMET_cut->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) >= 12 && h_ptoverMET_cut->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) <= 20) h_jetpt_ptoverMET_CR->Fill(jetPt[ijet], WEIGHT);
	      else if(h_ptoverMET_cut->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) >= 1 && h_ptoverMET_cut->GetXaxis()->FindBin(phoEt[ipho]/puppiMET) <= 11) h_jetpt_ptoverMET_SR->Fill(jetPt[ijet], WEIGHT);
	    }
	  }
