...
void xZgOOT(char* pathes){
  ...
  for (Long64_t ev = 0; ev < data.GetEntriesFast(); ev++){
    ...
    for(Int_t ipho=0; ipho<nINCPho; ipho++){
      if(abs(incPhoEta[ipho])<1.4442 && incPhoR9[ipho]>0.8  && incPhoEt[ipho]>0 && incPhoHoverE[ipho]<0.2){
      //if(abs(incPhoEta[ipho])<1.4442 && incPhoR9[ipho]>0.8  && incPhoEt[ipho]>210 && incPhoHoverE[ipho]<0.2 && pfMET>100 && npfjet>=2 && jetjetdR>0.4 ){
    ...
    fill = 0;
    for(Int_t ipho=0; ipho<nPho; ipho++){
      if(abs(phoSCEta[ipho])<1.4442 && phoR9Full5x5[ipho]>0.8 && phoCalibEt[ipho]>0 && phoHoverE[ipho]<0.2 ){
	//if(abs(phoSCEta[ipho])<1.4442 && phoR9Full5x5[ipho]>0.8 && phoCalibEt[ipho]>210 && phoHoverE[ipho]<0.2 && phohasPixelSeed[ipho]==1 && pfMET>100 && npfjet>=2 && jetjetdR>0.4 ){
	if(phoMIPTotEnergy[ipho]<4.9 && phohasPixelSeed[ipho]==1 && phoSigmaIEtaIEtaFull5x5[ipho]<0.01015 && metFilters==0){
