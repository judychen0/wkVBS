...
void xZg_data(char* pathes, Int_t isSM, Int_t isTrgs){
  //***********************Initialization***********************//
  ...
  for (Long64_t ev = 0; ev < data.GetEntriesFast(); ev++){
	...
	if(pho_sel(cutflow,15) == 1) h_phoEB_pt_diejtMass->Fill(phoEt[ipho], WEIGHT);
      ...
      if(pho_sel(cutflow,2) == 1) h_MET_phopt->Fill(puppiMET, WEIGHT);
      if(pho_sel(cutflow,3) == 1) h_MET_MphoID->Fill(puppiMET, WEIGHT);
      if(pho_sel(cutflow,5) == 1) h_MET_lepveto->Fill(puppiMET, WEIGHT);
      if(pho_sel(cutflow,6) == 1) h_MET_MET->Fill(puppiMET, WEIGHT);
      if(pho_sel(cutflow,7) == 1) h_MET_dphoMETPhi->Fill(puppiMET, WEIGHT);
      if(pho_sel(cutflow,8) == 1) h_MET_njet->Fill(puppiMET, WEIGHT);
      // if isSM == 0
      if(isSM == 0){
	if(pho_sel(cutflow,9) == 1) h_MET_jetpt->Fill(puppiMET, WEIGHT);
	if(pho_sel(cutflow,10) == 1) h_MET_jetjetdEta->Fill(puppiMET, WEIGHT);
	if(pho_sel(cutflow,11) == 1) h_MET_jetjetdPhi->Fill(puppiMET, WEIGHT);
	if(pho_sel(cutflow,12) == 1) h_MET_phojetdR->Fill(puppiMET, WEIGHT);
	if(pho_sel(cutflow,13) == 1) h_MET_ptoverMET->Fill(puppiMET, WEIGHT);
	if(pho_sel(cutflow,14) == 1) h_MET_mindJMETPhi->Fill(puppiMET, WEIGHT);
	if(pho_sel(cutflow,15) == 1) h_MET_diejtMass->Fill(puppiMET, WEIGHT);
      }
	...
	h_MET_cut->Fill(puppiMET, WEIGHT);
	h_METPhi_cut->Fill(puppiMETPhi, WEIGHT);
	...
	//     hJfake_MET_cut[ibin]->Fill(puppiMET, WEIGHT);
	//     hJfake_METPhi_cut[ibin]->Fill(puppiMETPhi, WEIGHT);
	...
	if(isSM == 0){
	  ...
	  //   hJfake_ptoverMET_cut[ibin]->Fill(phoEt[ipho]/puppiMET, WEIGHT);
      ...
      if(Nm1_sel(cutflow,6) == 1) h_MET_Nm1->Fill(puppiMET, WEIGHT);
	...
	h_MET_WgCR->Fill(puppiMET, WEIGHT);
	h_dijetMass_WgCR->Fill(dijetMass, WEIGHT);
      }
      
      // Fill h2 plot
      h2_phoEt_MET->Fill(puppiMET, phoEt[ipho], WEIGHT);
      //cout << "fill h2" << endl;
    }
  }
  //****************END LOOP**********************//
