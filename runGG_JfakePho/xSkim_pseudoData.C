...
void xSkim_pseudoData(char* pathes, char* PUpathes, char* IDpathes, char* PSVpathes, Int_t dcount, Int_t year, Int_t sig, Int_t ULsamples){
  ...
  TH1F* hbin_TruePho_dijetMass[Nbin];
  TH1F* hbin_FakePho_dijetMass[Nbin];
  TH1F* hbin_TruePho_dijetMass_cutsieie[Nbin];
  TH1F* hbin_FakePho_dijetMass_cutsieie[Nbin];
  ...
  for(Int_t jj=0; jj<Nbin; jj++){
    ...
    hbin_TruePho_dijetMass[jj] = new TH1F(Form("hbin_TruePho_dijetMass_%i", jj+1), "", 4, massbin);
    hbin_FakePho_dijetMass[jj] = new TH1F(Form("hbin_FakePho_dijetMass_%i", jj+1), "", 4, massbin);
    hbin_TruePho_dijetMass[jj]->Sumw2();
    hbin_FakePho_dijetMass[jj]->Sumw2();
    
    hbin_TruePho_dijetMass_cutsieie[jj] = new TH1F(Form("hbin_TruePho_dijetMass_cutsieie_%i", jj+1), "", 4, massbin);
    hbin_FakePho_dijetMass_cutsieie[jj] = new TH1F(Form("hbin_FakePho_dijetMass_cutsieie_%i", jj+1), "", 4, massbin);
    hbin_TruePho_dijetMass_cutsieie[jj]->Sumw2();
    hbin_FakePho_dijetMass_cutsieie[jj]->Sumw2();
  }

  TH1F* h_Truecount;
  TH1F* h_Fakecount;
  h_Truecount = new TH1F("h_TrueCount","", 4, massbin);
  h_Fakecount = new TH1F("h_FakeCount", "", 4, massbin);
  h_Truecount->Sumw2();
  h_Fakecount->Sumw2();
  ...
  for (Long64_t ev = 0; ev < data.GetEntriesFast(); ev++){
	...
	Int_t k = mc_phoid[nn];
	Float_t dr = deltaR(phoSCEta[ipho], phoPhi[ipho], mcEta_[k], mcPhi_[k]);
	...
	for(Int_t ibin=0; ibin<Nbin; ibin++){
	  if(phoEt[ipho] >= ptbin[ibin] && phoEt[ipho] < ptbin[ibin+1] && ipho == 0){
	    //cout << dijetMass << endl;
	    if(dcount == 0) hbin_TruePho_dijetMass[ibin]->Fill(dijetMass, EventWeight);
	    else hbin_FakePho_dijetMass[ibin]->Fill(dijetMass, EventWeight);
	  }
	}
      }
	...
	for(Int_t ibin=0; ibin<Nbin; ibin++){
	  if(phoEt[ipho] >= ptbin[ibin] && phoEt[ipho] < ptbin[ibin+1] && ipho == 0){
	    if(dcount == 0) hbin_TruePho_dijetMass_cutsieie[ibin]->Fill(dijetMass, EventWeight);
	    else hbin_FakePho_dijetMass_cutsieie[ibin]->Fill(dijetMass, EventWeight);
	  }
	}
      }

      // fill True/Fake count (dijetMass)
      if(Nm1_sel(bit,0) == 1){
	if(ipho == 0){
	  if(phoIsMatch[ipho]==0) h_Fakecount->Fill(dijetMass, EventWeight);
	  else if(phoIsMatch[ipho]==1) h_Truecount->Fill(dijetMass, EventWeight);
	}
      }
      
      // fill sieie, sieie loosed
      bit = ResetBit(3, bit);
      if(passNhIso[ipho] == 1 && passPhoIso[ipho] == 1 && passHoverE[ipho] == 1) bit = SetBit(3, bit);
	...
	for(Int_t ibin=0; ibin<Nbin; ibin++){
	  if(phoEt[ipho] >= ptbin[ibin] && phoEt[ipho] < ptbin[ibin+1] && ipho == 0){
	    if(phoIsMatch[ipho]==1) hbin_TruePho_sieie[ibin]->Fill(phoSigmaIEtaIEtaFull5x5[ipho], EventWeight);
	    if(phoIsMatch[ipho]==0) hbin_FakePho_sieie[ibin]->Fill(phoSigmaIEtaIEtaFull5x5[ipho], EventWeight);
	  }
	}
      }
     
      // // fill pseudo-Cand_sieie
      ...
      // 	for(Int_t ibin = 0; ibin < Nbin; ibin++){
      // 	  if(phoEt[ipho] >= ptbin[ibin] && phoEt[ipho] < ptbin[ibin+1] && ipho == 0){
      // 	    if(dijetMass>0) hbin_TruePho_dijetMass[ibin]->Fill(dijetMass, EventWeight);
      // 	  }
      // 	}
      // }
      
      // // fill pseudo-fake
      // if(dcount == 1 && phoIsMatch[ipho] == 0){
      // 	// fill fake_dijetMass
      // 	bit = ResetBit(3, bit);
      // 	if(passNhIso[ipho] == 1 && passPhoIso[ipho] == 1 && passHoverE[ipho] == 1 && passSieie[ipho] == 1) bit = SetBit(3, bit);
      // 	if(pho_sel(bit, 15) == 1){ 	
      // 	  for(Int_t ibin = 0; ibin < Nbin; ibin++){
      // 	    if(phoEt[ipho] >= ptbin[ibin] && phoEt[ipho] < ptbin[ibin+1] && ipho == 0){
      // 	      if(dijetMass>0) hbin_FakePho_dijetMass[ibin]->Fill(dijetMass, EventWeight);
      // 	    }
      // 	  }
      // 	}

      // 	//for(Int_t i=0; i<7; i++){bit = ResetBit(i+9,bit); bit = SetBit(i+9, bit);}
      // 	// fill fake_sieie
      // 	//bit = ResetBit(3, bit);
      // 	//if(passNhIso[ipho] == 1 && passPhoIso[ipho] == 1 && passHoverE[ipho] == 1) bit = SetBit(3, bit);
      // 	bit = ResetBit(8, bit);
      ...
      // 	      hbin_FakePho_sieie[ibin]->Fill(phoSigmaIEtaIEtaFull5x5[ipho], EventWeight);
      // 	    }
      // 	  }
      // 	}
      // }
  ...
  for(Int_t jj=0; jj<Nbin; jj++){
    ...
    hbin_FakePho_dijetMass[jj]->Write();
    hbin_FakePho_dijetMass_cutsieie[jj]->Write();
  }
  ...
  for(Int_t jj=0; jj<Nbin; jj++){
    ...
    hbin_TruePho_dijetMass[jj]->Write();
    hbin_TruePho_dijetMass_cutsieie[jj]->Write();
  }
